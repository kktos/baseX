...
export function advance() {
	// skip whitespaces
	while (
		source.idx < source.buffer.length &&
		ws.includes(source.buffer[source.idx])
	)
		source.idx++;
	...
	if (identiferChar0.includes(ch)) {
		while (
			source.idx < source.buffer.length &&
			identiferChars.includes(source.buffer[source.idx])
		)
			source.idx++;
	...
	if (numberChars.includes(ch)) {
		while (
			source.idx < source.buffer.length &&
			numberChars.includes(source.buffer[source.idx])
		)
			source.idx++;
		return;
	}
...
export function lexer(lookahead = false) {
	...
	if (isIdentifer) {
		while (
			currIdx < source.buffer.length &&
			identiferChars.includes(source.buffer[currIdx])
		)
			currIdx++;
	...
	if (isNumber) {
		...
		while (
			currIdx < source.buffer.length &&
			numberChars.includes(source.buffer[currIdx])
		) {
	...
	if (!lookahead) source.idx = currIdx + (isString ? 1 : 0);
...
export function tokenizer0(lookahead = false) {
	...
	if (!token) return -1;
	if (isString) return TOKENS._STRING;
